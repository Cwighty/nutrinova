...
[ApiController]
[Route("/be/[controller]")]
public class ChatController(NutrinovaDbContext context, INovaChatService chatService, ILogger<ChatController> logger)
  : ControllerBase
{
  [HttpGet("new-session")]
  ...
  [HttpGet("{sessionId}")]
  public async Task<ActionResult<IEnumerable<ChatMessageResponse>>> GetChats(Guid sessionId)
  {
    var userObjectId = User.GetObjectIdFromClaims();
    var customer = await context.Customers.FirstOrDefaultAsync(c => c.Objectid == userObjectId);
    if (customer is null || customer?.Id is null)
