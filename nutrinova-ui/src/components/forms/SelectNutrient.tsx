...
import { useGetNutrientsQuery, useGetUnitsQuery } from '@/app/(authorized)/food/foodHooks';
import { Alert, Autocomplete, Box, InputAdornment, Skeleton, TextField } from '@mui/material';
...
interface SelectNutrientProps {
  ...
  onNutrientAmountChange: (amount: number | null, unit: UnitOption | null) => void;
}
...
const SelectNutrient = (inputProps: SelectNutrientProps) => {
  ...
  if (nutrientOptionsIsError || unitOptionsIsError) {
    return <Alert severity="error">Error loading nutrient options, try again later</Alert>;
  }
  return (
    <>
