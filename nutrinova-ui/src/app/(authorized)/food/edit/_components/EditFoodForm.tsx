...
} from "@/app/(authorized)/recipes/create/_components/ServingSizeUnitField";
...
export const EditFoodForm = ({ foodId }: EditFoodFormProps) => {
  const {
    data: food,
  ...
  const [foodNutrientsAreValid, setFoodNutrientsAreValid] =
    useState<boolean>(false);
  const [newNutrient, setNewNutrient] = useState<EditFoodNutrientRequestModel>({
    nutrientId: 0,
    amount: 0,
    unitId: 0,
    nutrientName: "",
  ...
  const [editFoodFormState, setEditFoodForm] = useState<EditFoodRequestModel>({
    id: "",
    ...
    servingSizeUnit:
      unitOptions?.find((u) => u.abbreviation === food?.servingSizeUnit) ||
      null,
    foodNutrients:
      food?.foodNutrients?.map((fn) => {
        return {
  ...
  const handleAddNutrient = () => {
    ...
    const newNutrientList =
      editFoodFormState?.foodNutrients !== undefined
  ...
  const validateForm = () => {
    ...
    const validateNutrients = () => {
      const isValid =
        editFoodFormState?.foodNutrients?.every((n) => n.amount > 0) &&
        editFoodFormState.foodNutrients.length > 0;
  ...
  const handleNutrientAmountChange = (
    ...
    if (amount === "") {
      setFoodNutrientsAreValid(false);
      setIsValid(false);
    } else {
      if (editFoodFormState?.foodNutrients) {
        const updatedNutrientState = {
          ...editFoodFormState,
          foodNutrients: editFoodFormState?.foodNutrients.map((n) => {
  ...
  const handleNutrientDelete = (nutrientId: number) => {
    if (editFoodFormState?.foodNutrients) {
      const deletedNutrientState = {
        ...editFoodFormState,
        foodNutrients: editFoodFormState?.foodNutrients.filter((n) => {
  ...
  const handleSelectServingSizeUpdate = ({
    servingSize,
    servingSizeUnit,
    servingSizeUnitId,
  }: SelectNutrientWithUnitState) => {
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12}>
          <AddNutrientDialog
            ...
            setNewNutrient={(newNutrient) => setNewNutrient(newNutrient)}
          />
          {editFoodFormState.foodNutrients?.length === 0 && (
            <Alert severity="warning"> Foods need at least one nutrient </Alert>
          )}
          {editFoodFormState.foodNutrients?.map((fn, index) => (
            <>
              <EditNutrientListItem
                ...
                updateNutrient={(amount: number | "") =>
                  handleNutrientAmountChange(fn.nutrientId, amount)
                }
                inputOptions={{
                  error: !foodNutrientsAreValid,
                  helperText: !foodNutrientsAreValid
                    ? "Nutrients must have a value greater than 0"
                    : undefined,
                }}
              />
            </>
